...
class Chart extends WriterPart
{
    ...
    public function writeChart(\PhpOffice\PhpSpreadsheet\Chart\Chart $pChart, $calculateCellValues = true)
    {
		...
		$objWriter->writeAttribute('val', 'ja-JP');
		...
		//2019/09/20 AD START T.FUJITA
		if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
			//グラフの枠
			$objWriter->startElement('c:spPr');
			
			$objWriter->startElement('a:ln');
			$objWriter->writeAttribute('w',25424);
    ...
    private function writeLegend(XMLWriter $objWriter, Legend $legend = null)
    {
		...
		//2019/09/20 ADD START T.FUJITA
		$objWriter->startElement('a:r');
		$objWriter->startElement('a:rPr');
		$objWriter->writeAttribute('sz',830);
		$objWriter->writeAttribute('b',1);
		$objWriter->endElement();
		$objWriter->endElement();
		//2019/09/20 ADD END T.FUJITA
    ...
    private function writePlotArea(XMLWriter $objWriter, \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet, PlotArea $plotArea, Title $xAxisLabel = null, Title $yAxisLabel = null, Axis $xAxis = null, Axis $yAxis = null, GridLines $majorGridlines = null, GridLines $minorGridlines = null)
	{
		...
		if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
			...
			$objWriter->startElement('a:solidFill');
			$objWriter->startElement('a:srgbClr');
			$objWriter->writeAttribute('val', 'C0C0C0');
			$objWriter->endElement();
			$objWriter->endElement();
			...
			$objWriter->writeAttribute('cap','flat');
			$objWriter->writeAttribute('cmpd','sng');
			$objWriter->writeAttribute('algn','in');
    ...
    private function writeDataLabels(XMLWriter $objWriter, Layout $chartLayout = null)
    {
        ...
        //$objWriter->writeAttribute('val', 1);  //仮 値を表示
		$objWriter->endElement();
    ...
    private function writeCategoryAxis($objWriter, $xAxisLabel, $id1, $id2, $isMultiLevelSeries, Axis $yAxis)
    {
		...
		if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
			...
			$objWriter->writeAttribute('rtl', 0);
			$objWriter->startElement('a:defRPr');
			$objWriter->writeAttribute('sz',450);
			$objWriter->writeAttribute('b',1);
			$objWriter->endElement();
			$objWriter->endElement();
        ...
        //}
			...
			if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
				//Y軸ラベルテキストサイズ
				$objWriter->startElement('a:rPr');
				$objWriter->writeAttribute('sz',800);
				$objWriter->writeAttribute('b',1);
				$objWriter->endElement();
			}
            ...
            $objWriter->startElement('a:t');
			if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
				$objWriter->writeRawData(StringHelper::controlCharacterPHP2OOXML('月数'));
			}
            $objWriter->endElement();
...
/*             $layout = $xAxisLabel->getLayout();
    ...
    private function writeValueAxis($objWriter, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries, Axis $xAxis, GridLines $majorGridlines, GridLines $minorGridlines)
    {
		...
		//縦軸MAX値100
			...
			if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
				$objWriter->startElement('c:max');
				$objWriter->writeAttribute('val',100.0);
				$objWriter->endElement();
			}
		}
		//2019/09/20 AD END T.FUJITA
		...
		//縦軸MIN値0
			...
			if($this->getParentWriter()->getProgramId() == 'F_FLK0095'){
				$objWriter->startElement('c:min');
				$objWriter->writeAttribute('val',0.0);
				$objWriter->endElement();
			}
		}
		//2019/09/20 AD END T.FUJITA
        ...
        if ($yAxisLabel !== null) {
            ...
            $objWriter->startElement('a:bodyPr');
			//Y軸ラベルテキストの横表示
			if($this->getParentWriter()->getProgramId() == 'F_FLK0091'){
				$objWriter->writeAttribute('vert', 'horz');
			}elseif($this->getParentWriter()->getProgramId() == 'F_FLK0092'){
				$objWriter->writeAttribute('vert', 'horz');
			}
            $objWriter->endElement();
        ...
        foreach ($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
            ...
            if ($plotLabel) {
                ...
                if ($fillColor !== null) {
                    ...
                    //$objWriter->writeAttribute('val', '0FB7');
                    $objWriter->endElement();
                    $objWriter->endElement();
                    $objWriter->endElement();
                }
            ...
            if ($plotSeriesValues) {
                ...
                if ($plotSeriesMarker) {
					...
					$objWriter->writeAttribute('val','circle');
					//2019/09/20 AD END T.FUJITA
                    $objWriter->endElement();
                    ...
                    if ($plotSeriesMarker !== 'none') {
						...
						//2019/09/20 AD START T.FUJITA
                        //$objWriter->writeAttribute('val', 3);
						$objWriter->writeAttribute('val', 5);
						//2019/09/20 AD END T.FUJITA
                        $objWriter->endElement();
                    }
    ...
    private function writePlotSeriesValues($plotSeriesValues, XMLWriter $objWriter, $groupType, $dataType = 'str')
	{
        ...
        if ($plotSeriesValues->isMultiLevelSeries()) {
            ...
            $objWriter->startElement('c:f');
			//2019/09/20 AD START T.FUJITA
			if($this->getParentWriter()->getProgramId() == 'F_FLK0095' && $dataType == 'num'){
				$iSerArr = 0;
				$iSerArr = $this->getParentWriter()->getSerCnt();
				$objWriter->writeRawData('集計!$E$'.(1+(9*($iSerArr))).':$AN$'.(1+(9*($iSerArr))));
			}else{
				$objWriter->writeRawData($plotSeriesValues->getDataSource());
			}
			//2019/09/20 AD END T.FUJITA
