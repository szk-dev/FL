<?php
	...
	if($setupd<>"-1" and $strErrMsg == ""){
		...
		if($setupd == "1"){
			...
			//履歴適用
			$iRtnChk = $module_upd->fUpdHyokaFromRireki($aPara,$iYm,$iDate);
			if($iRtnChk  == -1){
				$strMsg = $module_sel->fMsgSearch("E001","品質評価集計表履歴データ適用処理エラー");
				goto proc_exit;
			}
		}
		//正常処理
		$strMsg = "N001 集計データを適用しました";
	}
	//出力処理
	...
	$aRireki = array();
	//集計履歴の取得
	$aRireki = $module_sel->fGetRenkeiRireki("F_FLK0091","F");
...
<!-- 個別javascript記述 -->
	...
	//適用ボタン
	function fUpdRireki(piYm,piDate){
			//確認メッセージ
			if(window.confirm('集計データを適用してもよろしいですか？')){
				document.form.action ="F_FLK0091.php?setupd=1&ym=" + piYm + "&date=" + piDate;
...
<TABLE border="0" bgcolor="#000066">
	<TBODY>
		<TR>
			<TD align="center" width="1000"><SPAN STYLE="FONT-FAMILY: Times New Roman;FONT-SIZE: 18PT;COLOR: FFFFFF;HEIGHT: 18PT;WIDTH: 400PT;FILTER:Alpha(,Style=uniform) DropShadow(Color=#660099,OffX=1,OffY=1) Shadow(Color=9933ff) ;">【品質評価集計表出力（コネクタ）】
			</SPAN></TD>
		</TR>
	</TBODY>
</TABLE>
			...
			$i = 0;
	  		while ($i < count($aRireki)){
				...
				if($aRireki[$i][5] <> ""){
					...
					echo "<INPUT type='button' value='適用' style='background-color : #fdc257;' onClick='fUpdRireki(\"".$aRireki[$i][3]."\",\"".$aRireki[$i][0]."\");'>";
				}
